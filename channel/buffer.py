...
class BaseBuffer(object):
    ...
    def __init__(self, bufferSize: int = 0, verbose: bool = False) -> None:
        """
        Assign variables.
...
class FIFOBuffer(BaseBuffer):
    ...
    def __init__(self, bufferSize: int = 0, delay: int = 0, verbose: bool = False) -> None:
        """
        bufferSize: int, the maximum number of objects that can be stored in the buffer.
        delay: int,propagation delay
        verbose: bool, whether to print
        """
        super(FIFOBuffer, self).__init__(bufferSize=bufferSize, verbose=verbose)
    ...
    def enqueue(self, packet: Packet, time: int = 0) -> bool:
        ...
        if self.verbose:
            print("[+] @{time} Enqueue {curSize}/{cap} pkt {pkt}".format(
    ...
    def dequeue(self, time: int = -sys.maxsize) -> List[Packet]:
        ...
        pktList = []
        timeThres = time - self.delay # the threshold of time before which a packet should be poped out
        ...
        if self.verbose:
            if(len(pktList)):
                print("[+] @{time} Dequeu {nPkt} pkts {curSize}/{cap}".format(time=time,
                      nPkt=len(pktList), curSize=self.nPktsInBuf, cap=self.bufferSize))
            else:
                print("[-] @{time} Empty queue {curSize}/{cap}".format(time=time,
...
class PriorityBuffer(BaseBuffer):
    ...
    def __init__(self, bufferSize: int = 0, rng: BaseRNG=RangeUniform(2, 10), verbose: bool = False) -> None:
        super(PriorityBuffer, self).__init__(bufferSize=bufferSize, verbose=verbose)
    ...
    def enqueue(self, packet: Packet, time: int) -> bool:
        ...
        if self.verbose:
            print("[+] @{time} Enqueue {curSize}/{cap} pkt {pkt} dequeue >= {dequeueTime}".format(
...
if __name__ == "__main__":
    ...
    buffer = FIFOBuffer(bufferSize=3, delay=2, verbose=True)
    ...
    import random
    random.seed(0)
    buffer = PriorityBuffer(bufferSize=5, rng=RangeUniform(3, 10), verbose=True)
    ...
    assert buffer.isFull() == True, "buffer should be full now"
    # except to see full buffer message
    ...
    pktList = buffer.dequeue(time=10) # should see four packets with pid 1 3 0 2
    for pkt in pktList: print(pkt)
    assert buffer.isEmpty() == True, "buffer should be empty now"
